import numpy as np
from cnn.layers import Conv2DLayer, ReLULayer, MaxPoolingLayer, FullyConnectedLayer
...
class CNN:
    ...
    def backward(self, dL_dinput):
        ...
        # Get the batch size from the shape of dL_dinput
        batch_size = dL_dinput.shape[0]

        # Reshape the output based on the batch size
        dL_dinput = dL_dinput.reshape(batch_size, 14, 14, 128)
        print("After reshape", dL_dinput.shape)
...
#         return conv3_output_size * conv3_output_size * 128
...
#         dL_dinput = dL_dinput.reshape((-1, 14, 14, 128))  # Reshape dynamically based on batch size
#         print("After reshape", dL_dinput.shape)
        
#         dL_dinput = self.relu.backprop(dL_dinput)
#         print("After relu backward", dL_dinput.shape)
        
#         dL_dinput, dL_dweights3, dL_dbias3 = self.conv3.backward(dL_dinput)
#         print("After conv3 backward", dL_dinput.shape)
        
#         dL_dinput = self.pool1.backward(dL_dinput)
#         print("After pool1 backward", dL_dinput.shape)
        
#         dL_dinput, dL_dweights2, dL_dbias2 = self.conv2.backward(dL_dinput)
#         print("After conv2 backward", dL_dinput.shape)
        
#         dL_dinput, dL_dweights1, dL_dbias1 = self.conv1.backward(dL_dinput)
#         print("After conv1 backward", dL_dinput.shape)
        
#         # Collect gradients
#         gradients = {
#             'conv1': (dL_dweights1, dL_dbias1),
#             'conv2': (dL_dweights2, dL_dbias2),
#             'conv3': (dL_dweights3, dL_dbias3),
#             'fc1': (self.fc1.dL_dweights, self.fc1.dL_dbias),
#         }
...
#     def params(self):
#         return (
#             list(self.conv1.params) +
#             list(self.conv2.params) +
#             list(self.conv3.params) +
#             list(self.fc1.params)
#         )
